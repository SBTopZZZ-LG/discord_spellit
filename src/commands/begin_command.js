...
const {
	duration1,
	rounds1,
} = require("../utils/interaction_options_parser.util");
...
async function handler(interaction) {
	const duration =
		duration1[interaction.options.get("duration")?.value ?? "t_30s"];
	const rounds = rounds1[interaction.options.get("rounds")?.value ?? "r_10"];
	const multipleGuesses =
		interaction.options.get("multiple_guesses")?.value ?? false;
			...
			embeds: [
				new EmbedBuilder()
					...
					.setFields([
						...
						{
							name: "Multiple guesses allowed",
							value: multipleGuesses ? "Yes" : "No",
						},
					]),
			],
		});
	...
	} else interaction.reply("The challenge is already started in this guild!");
}
